 ...
 */
export function areAdjacentFacets(
  facet1: FacetName,
  facet2: FacetName
): boolean {
...
export function getPreviousFacet(facet: FacetName): FacetName {
  const currentIndex = FACET_ORDER.indexOf(facet);
  const prevIndex =
    (currentIndex - 1 + FACET_ORDER.length) % FACET_ORDER.length;
 ...
 */
export function getFacetRange(
  startFacet: FacetName,
  endFacet: FacetName
): FacetName[] {
...
export function useFacetState(initialFacet: FacetName = "ontology") {
  const [currentFacet, setCurrentFacet] =
    React.useState<FacetName>(initialFacet);
...
/**
 * Enhanced version of getFacetClass that supports custom variant classes (border-light, hover-light, etc.)
 * for better CSS control. This handles non-standard Tailwind classes we've added for the project.
 */
export function getFacetStyle(
  facet: FacetName | ColorType,
  variant: string
): string {
  // Handle custom variant cases like "border-light" or "hover-light"
  if (variant.includes("-")) {
    const [baseType, modifier] = variant.split("-");
    return `${baseType}-facet-${facet.toLowerCase()}-${modifier}`;
  }

  // Fall back to standard behavior
  return `${variant}-facet-${facet.toLowerCase()}`;
}
