 ...
 * 
 * Implements EXTRE    // Band contrast boost: Stronger lightness variation for maximum band separation
    ...
    const finalLightness = clamp(lightness + lightnessBandBoost, 0, 100);tra-vivid, maximum-contrast ROYGBIV colors with ultimate band differences.
...
export const OPTIMIZED_LCH_RANGES: Record<
  FacetName,
  {
...
> = {
  Ontology: {
    // Violet - EXTREME contrast range for maximum differentiation
  ...
  Epistemology: {
    // Indigo - EXTREME contrast range for maximum differentiation
    lightness: { min: 25, max: 95 },
    chroma: { min: 40, max: 140 },
  },
  Praxeology: {
    // Blue - EXTREME contrast range for maximum differentiation
    lightness: { min: 25, max: 95 },
    chroma: { min: 40, max: 140 },
  },
  Axiology: {
    // Green - Extreme range while maintaining natural appearance
  ...
  Teleology: {
    // Red - EXTREME contrast range for maximum differentiation
 ...
 */
export function calculateBandColor(
  facetName: FacetName,
  score: number
): string {
  ...
  try {
    ...
    const lightness =
      ranges.lightness.min +
      sCurveScore * (ranges.lightness.max - ranges.lightness.min);
    const chromaValue =
      ranges.chroma.min + sCurveScore * (ranges.chroma.max - ranges.chroma.min);
    ...
    const finalLightness = Math.max(
      0,
      Math.min(100, lightness + lightnessBandBoost)
    );
 ...
 */
export function generateTriangleColorPalette(
  domainScores: DomainScore[]
): Record<FacetName, string> {
  const colorPalette: Record<FacetName, string> = {} as Record<
    FacetName,
    string
  >;
 ...
 */
export function getFacetScoreColorLCH(
  facetName: FacetName,
  score: number
): string {
 ...
 */
export function getFacetColorInfo(
  facetName: FacetName,
  score: number
): {
  hex: string;
  ...
  const lightness =
    ranges.lightness.min +
    enhancedScore * (ranges.lightness.max - ranges.lightness.min);
  const chromaValue =
    ranges.chroma.min + enhancedScore * (ranges.chroma.max - ranges.chroma.min);
 ...
 */
export function testFacetColorRange(
  facetName: FacetName
): Array<{ score: number; color: string }> {
