 ...
 */
export function interpolateFacetColor(facetName: FacetName, score: number): string {
...
export function colorScale(colors: string[], score: number): string {
  ...
  const color2 = hexToRgb(colors[Math.min(segmentIndex + 1, colors.length - 1)]);
 ...
 */
export function getFacetScoreColorLegacy(facetName: FacetName, score: number): string {
...
export const getDominantFacet = (scores: DomainScore[]): FacetName => {
  if (!scores || !Array.isArray(scores) || scores.length === 0) {
    return Array.isArray(FACET_NAMES) && FACET_NAMES.length > 0 ? FACET_NAMES[0] : 'Ontology'; // Default to the first facet name if no scores
  }
  const validScores = scores.filter(s => s && s.facetName && typeof s.score === 'number' && FACET_NAMES.includes(s.facetName as FacetName));
