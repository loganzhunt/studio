 ...
 */
export function calculateRawDomainScore(domainResponses: (number | null | undefined)[]): number {
  let rawScore = 0;
 ...
 */
export function calculateAllDomainScores(allAnswers: AssessmentAnswers, facetsConfig: Record<FacetName, Facet>): DomainScore[] {
  ...
  for (const facetName of FACET_NAMES) {
    ...
    if (answeredQuestions > 0) {
      ...
      if (maxPossible === minPossible) { // all answers were 1 or all were 5, or only one question
        ...
        normalizedScore = (rawScore - minPossible) / (maxPossible - minPossible);
