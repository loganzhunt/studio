...
import { FACET_INFO, getFacetClass, getFacetClasses, type FacetName } from '@/lib/facet-colors';
import { GlassCard, PrismButton, HeroShimmer, SpectrumBar } from '@/components/glass-components';
    ...
    <div className="flex flex-col">
      ...
      <main className="flex-1">
          ...
          <div className="container mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {howItWorksSteps.map((step, index) => {
                ...
                const stepFacets: FacetName[] = ['epistemology', 'cosmology', 'axiology'];
                ...
                return (
                  ...
                  >
                    <div className="mb-8">
                      <div className={`inline-flex p-4 rounded-2xl ${getFacetClass('bg', facetKey, '100')} mb-4`}>
                        <step.icon className={`h-12 w-12 ${getFacetClass('text', facetKey, '600')}`} />
                      </div>
                    </div>
                    <h3 className={`text-2xl font-semibold mb-4 ${getFacetClass('text', facetKey, '700')}`}>
                      {step.title}
                    </h3>
        ...
        <section className="py-16 md:py-24 bg-card/30">
          <div className="container mx-auto">
            <h2 className="text-3xl md:text-4xl font-bold text-center mb-6">Featured Worldviews</h2>
            ...
            <div className="flex justify-center">
              <Carousel
                opts={{
                  align: "start",
                  loop: (featuredWorldviewsData && Array.isArray(featuredWorldviewsData) && featuredWorldviewsData.length > 3) || false,
                }}
              ...
              >
                <CarouselContent className="-ml-4">
                  {(featuredWorldviewsData || []).map((entry) => {
                    ...
                    return (
                      <CarouselItem key={entry?.id || Math.random()} className="pl-4 sm:basis-1/2 lg:basis-1/3 xl:basis-1/4">
                          ...
                          <Card className="flex flex-col h-full overflow-hidden hover:shadow-lg transition-shadow duration-300 ease-in-out glassmorphic-card">
                            <CardHeader className="p-6 text-center">
                               <div className="flex items-center justify-center gap-2 mb-1">
                                {entry?.icon && <span className={`text-3xl ${getFacetClass('text', facetKey, '600')}`}>{entry.icon}</span>}
                                <CardTitle className={`text-xl line-clamp-1 ${getFacetClass('text', facetKey, '700')}`}>
                            ...
                            <CardContent className="flex-grow flex flex-col justify-center items-center pt-2 pb-4">
                              <TriangleChart scores={entry?.domainScores || []} worldviewName={entry?.title || 'Unknown'} width={180} height={156} className="!p-0 !bg-transparent !shadow-none !backdrop-blur-none mb-3" />
                            </CardContent>
                  ...
                  >
                    <AccordionTrigger className="p-6 text-lg hover:no-underline group">
                      <div className="flex items-center gap-4">
                        <div className={`w-10 h-10 rounded-lg ${getFacetClass('bg', facetKey, '500')} flex items-center justify-center group-hover:scale-110 transition-transform`}>
                          <FacetIcon facetName={facet.name} className="h-5 w-5 text-white" />
                        </div>
                        <div className="text-left">
                          <span className={`${getFacetClass('text', facetKey, '700')} font-semibold`}>
                            {facet.name}
                          </span>
                    ...
                    <AccordionContent className="px-6 pb-6">
                      <div className={`border-l-4 ${getFacetClass('border', facetKey, '400')} pl-4 mb-4`}>
                        <p className="text-muted-foreground mb-3">{facet.description}</p>
                        {facetInfo && (
                          <p className={`text-sm ${getFacetClass('text', facetKey, '600')} font-medium`}>
                      ...
                      <Button 
                        variant="outline" 
                        size="sm" 
                        className={`${getFacetClass('border', facetKey, '400')} ${getFacetClass('text', facetKey, '700')} hover:${getFacetClass('bg', facetKey, '100')}`}
                        asChild
                      >
        ...
        <div className="container mx-auto px-4 text-muted-foreground">
          <div className="flex flex-col items-center justify-center text-center">
            ...
            <div className="mt-2 flex flex-col sm:flex-row gap-2 sm:gap-6 items-center">
              <Link href="/about" className="text-sm text-primary hover:underline transition-colors duration-300 ease-in-out">
                Learn More About the Meta-Prism Model
              </Link>
              <Link href="/design-system" className="text-sm text-primary hover:underline transition-colors duration-300 ease-in-out">
                Design System
              </Link>
            </div>
          </div>
