...
import { SPECTRUM_LABELS, getFacetColorHsl, DOMAIN_COLORS } from '@/lib/colors';
...
export default function FacetDeepDivePage() {
  ...
  const facetKey = capitalizedFacetName.toLowerCase() as any;

  return (
    <div className="container mx-auto py-8 space-y-10">
      {/* Facet Header */}
      <header className={`text-center border-b-2 pb-6 mb-8 relative ${getFacetClass('border', facetKey, '500')}`}>
        <FacetIcon facetName={facet.name} className="w-20 h-20 mx-auto mb-4" />
        <h1 className={`text-4xl font-bold mb-2 ${getFacetClass('text', facetKey, '700')}`}>
          Deep Dive: The {facet.name} Facet
        </h1>
        <p className="text-xl text-muted-foreground">{facet.tagline}</p>
        <div 
          className={`absolute bottom-[-2px] left-0 w-full h-[3px] ${getFacetClass('bg', facetKey, '500')}`}
          style={{ background: `linear-gradient(to right, transparent, var(--facet-${facetKey}-500), transparent)` }}
        />
      </header>
      ...
      <section className="space-y-8">
        <Card className="border-primary/30 shadow-lg">
          <CardHeader>
            <CardTitle className={`text-2xl ${getFacetClass('text', facetKey, '700')}`}>
