...
export function mapRawDataToCodexEntries(rawItems: any[]): CodexEntry[] {
  ...
  return rawItems.map((item: any, index: number) => {
    ...
    const validCategories: CodexEntry['category'][] = ['philosophical', 'religious', 'archetypal', 'custom'];
      ...
      <Card className="flex flex-col overflow-hidden glassmorphic-card hover:shadow-primary/20 transition-shadow duration-300 ease-in-out h-full">
        <CardHeader className="p-6 text-center">
          <div className="flex items-center justify-center gap-2 mb-2">
            {entry.icon && <span className={`text-3xl ${getFacetClass('text', dominantFacet.toLowerCase() as any, '600')}`}>{entry.icon}</span>}
            <CardTitle className={`text-xl line-clamp-1 ${getFacetClass('text', dominantFacet.toLowerCase() as any, '700')}`}>
              {entry.title}
            </CardTitle>
          </div>
        ...
        <CardContent className="flex-grow flex flex-col justify-center items-center pt-2 pb-4">
          <TriangleChart scores={entry.domainScores} worldviewName={entry.title} width={180} height={156} className="!p-0 !bg-transparent !shadow-none !backdrop-blur-none mb-4" />
        </CardContent>
      ...
      <Card className="mb-8 p-4 sm:p-6 glassmorphic-card">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
          <div className="md:col-span-2">
            <label htmlFor="search-codex" className="block text-sm font-medium text-muted-foreground mb-2">Search Codex</label>
            <Input
              id="search-codex"
              type="text"
          ...
          <div>
            <label htmlFor="sort-by" className="block text-sm font-medium text-muted-foreground mb-2">Sort By</label>
            <Select value={sortBy} onValueChange={setSortBy}>
              <SelectTrigger id="sort-by" className="bg-muted/80 border-border/60 hover:bg-muted/90">
          ...
          <div className="flex flex-wrap gap-2">
            {allCategories.map(cat => (
              <Button
                ...
                size="sm"
                onClick={() => setActiveCategory(cat as CodexEntry['category'] | "all")}
            ...
            <TooltipProvider>
              <div className="p-6">
                <SheetHeader className="mb-8">
                  <div className="flex justify-between items-start">
                    <div className="flex items-center gap-3">
                      {selectedEntry.icon && <span className={`text-4xl ${getFacetClass('text', getDominantFacet(selectedEntry.domainScores).toLowerCase() as any, '600')}`}>{selectedEntry.icon}</span>}
                      <div>
                        <SheetTitle className={`text-3xl mb-2 ${getFacetClass('text', getDominantFacet(selectedEntry.domainScores).toLowerCase() as any, '700')}`}>
                          {selectedEntry.title}
                        </SheetTitle>
                ...
                <div className="mb-8 flex justify-center">
                   <TriangleChart scores={selectedEntry.domainScores} worldviewName={selectedEntry.title} width={250} height={217} className="mx-auto !p-0 !bg-transparent !shadow-none !backdrop-blur-none" />
                </div>
                ...
                <div className="space-y-6 mb-8">
                  <h3 className="text-xl font-semibold text-foreground mb-4 border-b border-border/30 pb-2">Facet Breakdown</h3>
                  {FACET_NAMES.map(facetName => {
                    ...
                    return (
                      <div key={facetName} className="p-4 rounded-md border border-border/30 bg-background/40 space-y-2">
                        ...
                        <p className="text-xs text-muted-foreground italic mb-2">{facetConfig?.tagline || "..."}</p>
                        <p className="text-sm text-muted-foreground mb-3">{facetSummary}</p>
                        ...
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <div className="w-full h-6 rounded bg-muted/30 relative mt-2" aria-label={`${facetName} spectrum: ${spectrumPoleLabels.left} to ${spectrumPoleLabels.right}. Score: ${Math.round(score * 100)}%`}>
                              <div 
                                className={`h-full rounded ${getFacetClass('bg', facetName.toLowerCase() as any, '500')}`}
                              />
                              <div
