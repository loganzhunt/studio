...
// Combine existing data with the new batch, ensuring unique names.
// Note: This simple check might not be sufficient if casing differs but names are meant to be the same.
const existingNames = new Set(existingRawCodexData.map(item => item.name.toLowerCase()));
const uniqueNewEntries = newRawCodexDataBatch.filter(newItem => !existingNames.has(newItem.name.toLowerCase()));

const rawCodexData = [...existingRawCodexData, ...uniqueNewEntries];


const mapRawDataToCodexEntries = (rawData: any[]): CodexEntry[] => {
  return rawData.map(item => {
    const domainScoresArray: DomainScore[] = FACET_NAMES.map(facetName => {
      // Try to get score with capitalized key first (matching FacetName), then lowercase
      const scoreValue = item.domainScores[facetName] ?? item.domainScores[facetName.toLowerCase()];
      return {
        facetName,
        score: scoreValue !== undefined ? Number(scoreValue) : 0.5 // Default score if missing, ensure number
      };
    });

    const processedFacetSummaries: { [K_FacetName in FacetName]?: string } = {};
    if (item.facetSummary) {
      for (const rawKey in item.facetSummary) {
        const capitalizedKey = FACET_NAMES.find(fn => fn.toLowerCase() === rawKey.toLowerCase());
        if (capitalizedKey) {
          processedFacetSummaries[capitalizedKey] = item.facetSummary[rawKey as keyof typeof item.facetSummary];
        }
      }
    }
    ...
    if (currentTags.length > 0) {
        if (currentTags.includes("philosophical")) category = "philosophical";
        else if (currentTags.includes("religious")) category = "religious";
        else if (currentTags.includes("archetypal")) category = "archetypal";
        else if (currentTags.includes("spiritual") && category === "custom") category = "philosophical";
        else if (currentTags.includes("mystical") && category === "custom") category = "philosophical"; // Adding mystical as philosophical
        else if (currentTags.includes("scientific") && category === "custom") category = "philosophical";
        else if (currentTags.includes("cultural") && category === "custom") category = "philosophical";
        else if (currentTags.includes("indigenous") && category === "custom") category = "philosophical"; // Often spiritual/philosophical
    }
    ...
    return {
      ...
      category,
      tags: currentTags,
      isArchetype: currentTags.includes("archetypal") || false,
      createdAt: new Date().toISOString(), // Consider if raw data has a date
    };
  });
};
...
export default function CodexPage() {
  ...
  const filteredAndSortedEntries = useMemo(() => {
    ...
    // Add more sorting options as needed, e.g., by relevance (default, no explicit sort here yet)
    return entries;
