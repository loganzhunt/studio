...
export const WorldviewProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const loadStateFromLocalStorage = useCallback(() => {
    try {
      if (currentUser?.uid) {
        ...
        const storedAnswers = localStorage.getItem(`metaPrismAssessmentAnswers_${userKey}`);
        if (storedAnswers) setAssessmentAnswers(JSON.parse(storedAnswers));

        const storedScores = localStorage.getItem(`metaPrismDomainScores_${userKey}`);
        if (storedScores) {
        ...
        const storedSavedWorldviews = localStorage.getItem(`metaPrismSavedWorldviews_${userKey}`);
        if (storedSavedWorldviews) setSavedWorldviews(JSON.parse(storedSavedWorldviews));

        const storedFacetSelections = localStorage.getItem(`metaPrismFacetSelections_${userKey}`);
  ...
  const hasAssessmentBeenRun = Object.keys(assessmentAnswers).length > 0 && 
    FACET_NAMES.some(facetName => 
      Object.keys(assessmentAnswers).some(key => key.startsWith(facetName.toLowerCase()))
    );
