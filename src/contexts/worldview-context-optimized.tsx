...
export const WorldviewProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  // Local sign-in method for development/testing
  const signInLocally = useCallback(
    async (email: string, password: string) => {
      try {
        dispatch({ type: "SET_LOADING", payload: true });

        // For development purposes, create a mock local user
        ...
        toast({
          ...
          description: `Welcome ${localUser.displayName}!`,
        });
      } catch (error: any) {
        console.error("Local sign-in error:", error);
        const message = "Failed to sign in locally.";
        dispatch({ type: "SET_ERROR", payload: message });
        toast({ title: "Error", description: message, variant: "destructive" });
      }
    },
